...
class SanBotRouter(Star):
    ...
    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE, priority=maxsize - 100)
    async def route_group_message(self, event: AstrMessageEvent):
        ...
        if is_sanbot_command(text):
            logger.debug("SanBot router delegated business command to SanBot: %s", text[:80])
            event.stop_event()
            return
