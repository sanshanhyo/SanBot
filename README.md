...
`.env.example` 已经为每一项配置写好中文注释。首次部署时复制为 `.env`，再按自己的环境修改即可。

字段说明：

安全建议：先用 `ALLOWED_GROUP_IDS` 限制机器人能服务的群，再按功能配置 `*_ALLOWED_GROUP_IDS`。功能级白名单留空表示“不在全局白名单之外再额外限制”；如果某个功能风险较高，例如 MissAV 外部播放入口，建议显式填写该功能白名单。
...
| `BOT_ALLOWED_GROUP_IDS` | 兼容旧版本的群白名单；`ALLOWED_GROUP_IDS` 为空时才读取 |
...
| `ENABLE_JM_DOWNLOAD` | 是否启用 JM 下载，默认 `true` |
| `JM_DOWNLOAD_ALLOWED_GROUP_IDS` | JM 下载功能白名单群，留空表示不额外限制 |
| `ENABLE_JM_SEARCH` | 是否启用 JM 关键词搜索，默认 `true`；未设置时兼容 `ENABLE_SEARCH` |
| `ENABLE_SEARCH` | 旧版 JM 关键词搜索开关，同时仍控制后端 `/api/search` |
| `JM_SEARCH_ALLOWED_GROUP_IDS` | JM 搜索功能白名单群，留空表示不额外限制 |
| `ENABLE_JM_RANKING` | 是否启用 JM 日榜、周榜、月榜，默认 `true` |
| `JM_RANKING_ALLOWED_GROUP_IDS` | JM 排行榜功能白名单群，留空表示不额外限制 |
...
| `ENABLE_JAVLIBRARY` | JAV 元数据总开关，默认 `true` |
| `ENABLE_JAV_QUERY` | 是否启用番号详情查询，默认 `true`；未设置时兼容 `ENABLE_JAVLIBRARY` |
| `JAV_QUERY_ALLOWED_GROUP_IDS` | 番号详情查询白名单群，留空表示不额外限制 |
| `ENABLE_AV_SEARCH` | 是否启用 AV 标题搜索，默认 `true` |
| `AV_SEARCH_ALLOWED_GROUP_IDS` | AV 标题搜索白名单群，留空表示不额外限制 |
| `ENABLE_ACTOR_SEARCH` | 是否启用演员搜索，默认 `true` |
| `ACTOR_SEARCH_ALLOWED_GROUP_IDS` | 演员搜索白名单群，留空表示不额外限制 |
| `ENABLE_DB_RANKING` | 是否启用 JavDB 日榜、周榜、月榜，默认 `true` |
| `DB_RANKING_ALLOWED_GROUP_IDS` | JavDB 排行榜白名单群，留空表示不额外限制 |
...
| `JAV_RESOURCE_PAGE_ALLOWED_GROUP_IDS` | JavDB 资源页白名单群，留空表示不额外限制 |
| `ENABLE_JAV_TRAILER` | 是否允许回复“预告片”发送预告片，默认 `true` |
| `JAV_TRAILER_ALLOWED_GROUP_IDS` | JavDB 预告片白名单群，留空表示不额外限制 |
| `JAV_TRAILER_FFMPEG_PATH` | 转换 m3u8 预告片所用的 ffmpeg 路径，默认 `ffmpeg` |
| `JAV_TRAILER_CONVERT_TIMEOUT_SECONDS` | 预告片转 MP4 超时时间，默认 `180` 秒 |
| `JAV_TRAILER_MAX_BYTES` | 预告片 MP4 最大体积，默认 `104857600` 字节 |
| `JAV_TRAILER_COOKIE` | 预告片 m3u8 请求 Cookie，留空时复用 `JAVLIBRARY_COOKIE` |
| `JAV_TRAILER_IMPERSONATE` | 预告片 HLS 下载使用的 curl-cffi 浏览器指纹，默认复用 `JAVLIBRARY_IMPERSONATE`，再默认 `random` |
| `ENABLE_JAV_STILLS` | 是否允许回复“剧照”发送剧照预览，默认 `false` |
| `JAV_STILLS_ALLOWED_GROUP_IDS` | 剧照预览白名单群，留空表示不额外限制 |
...
| `JAV_STILLS_PDF_ALLOWED_GROUP_IDS` | 剧照 PDF 白名单群，留空表示不额外限制 |
...
| `TG_MIRROR_ALLOWED_GROUP_IDS` | Telegram 频道镜像白名单群，留空表示不额外限制 |
| `ENABLE_TG_AUTO_FETCH` | 是否启用 Telegram 自动拉取，默认 `false` |
| `TG_AUTO_FETCH_GROUP_IDS` | Telegram 自动拉取固定群列表；留空时使用后端已绑定 TG 频道的群 |
| `TG_AUTO_FETCH_INTERVAL_SECONDS` | Telegram 自动拉取间隔，默认 `3600` 秒，即 1 小时 |
| `TG_AUTO_FETCH_LIMIT` | Telegram 自动拉取每个群单次数量，默认 `5`，最大 `10` |
...
| `ENABLE_HISTORY` | 是否启用任务历史查询，默认 `true` |
| `HISTORY_ALLOWED_GROUP_IDS` | 任务历史查询白名单群，留空表示不额外限制 |
| `ENABLE_ADMIN_COMMANDS` | 是否启用管理员运维命令，默认 `true` |
| `ADMIN_ALLOWED_GROUP_IDS` | 管理员运维命令白名单群，留空表示不额外限制 |
...
后端会记录已转发的 `channel_id + message_id`，避免重复转发；单个媒体超过 `TG_MAX_FILE_BYTES` 会跳过。请只绑定你有权访问和转发内容的频道。

也可以开启自动拉取。自动拉取默认 1 小时一次，只会发送新发现的图片/视频；如果没有新内容，不会在群内发送“没有新内容”之类的提示：

```env
ENABLE_TG_AUTO_FETCH=true
TG_AUTO_FETCH_INTERVAL_SECONDS=3600
TG_AUTO_FETCH_LIMIT=5
```

`TG_AUTO_FETCH_GROUP_IDS` 留空时，Bot 会从后端读取已绑定 TG 频道的群；如果你想固定只同步某些群，也可以用英文逗号指定群号。
